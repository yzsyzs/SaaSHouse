          ...
          <div class="events-day" v-for="day in week" track-by="$index"
            ...
            <div class="event-box">
              <p class="event-item" v-for="event in day.events" v-show="event.cellIndex <= eventLimit"
                ...
                <span v-if='isClassSetToVueFullCalendar === "true"' :class="{groupCurriculum_private_sign_c: event && event.data.courseType === 'PERSONALCOURSE', groupCurriculum_group_sign_c: event && event.data.courseType === 'GROUPCOURSE'}">&nbsp;</span>
                <span style="padding-left: 4px;">{{isBegin(event, day.date, day.weekDay)}}</span>
              </p>
        ...
        <div class="more-body">
          <ul class="body-list">
            <li
              ...
              @click="eventClick(event,$event)">
              <span v-if='isClassSetToVueFullCalendar === "true"' :class="{groupCurriculum_private_sign_c: event && event.data.courseType === 'PERSONALCOURSE', groupCurriculum_group_sign_c: event && event.data.courseType === 'GROUPCOURSE'}">&nbsp;</span>
              <span style="padding-left: 4px;">{{event.title}}</span>
            </li>
          </ul>
        </div>
      </div>
...
<script type="text/babel">
  ...
  export default {
    ...
    created () {
      ...
      this.isClassSetToVueFullCalendar = localStorage.isClassSetToVueFullCalendar
      // this.events = events
    },
    data () {
      return {
