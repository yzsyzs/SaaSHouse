    ...
    methods : {
      goPrev () {
        ...
        var h = this.headDate,
            t = this.titleFormat,
            mn = this.monthNames,
            title = dateFunc.format(this.headDate, this.titleFormat, this.monthNames),
            y = title.split('年')[0],
            m = title.split('月')[0].split('年')[1]
        // 注意这里localStorage取处理的值是字符串
        if((this.checkIsOutOfDate(y + '-' + m) && this.isClassSetToVueFullCalendar !== 'false')) {
          this.headDate = h
          this.titleFormat = t
          this.monthNames = mn
       ...
       */
      checkIsOutOfDate(c) {
        let self = this, date = new Date(), y = date.getFullYear(), m = date.getMonth()
        m = m === 0 ? 12 : m + 1
        return (new Date(c).getTime() <= new Date(y + '-' + m).getTime())
      },
